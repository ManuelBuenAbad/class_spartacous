...
#include "dei_rkck.h"
#include "parser.h"
//spartacous: importing new headers with ad-hoc functions
#include "bessel.h"
#include "sigmoid.h"
#include "numericx.h"
//spartacous
...
//spartacous
/** list of possible statistics for the dark radiation freezing out with which the PAcDM interacts in the SPartAcous model.
*/
enum dark_radiation_sub_statistic {boson, fermion};
//spartacous
...
struct background
{
  ...
  //spartacous: new variables and parameters
  enum dark_radiation_sub_statistic dr_sub_stat;// statistics of the DR subcomponent with which the DM interacts
  double Omega0_dm_tot;// abundance of total DM (vanilla CDM + PAcDM)
  double Omega0_pacdm;// abundance of PAcDM (chi_2)
  double m_pacdm;// [GeV] mass of PAcDM (chi_2)
  double Omega0_pacdr;// abundance of PAcDR TODAY (A' only)
  double N_UV;// extra neutrino families in PAcDR in the UV
  double N_IR;// extra neutrino families in PAcDR in the IR
  double g_UV;// degrees of freedom in PAcDR in the UV: including factors of 7/8 for fermions
  double g_IR;// degrees of freedom in PAcDR in the IR: including factors of 7/8 for fermions
  double r_g;// step from the decrease in the degrees of freedom in PAcDR in going from UV --> IR
  double g_sub;// d.o.f. in the PAcDR subcomponent responsible for the DM-DR interactions. In our paper's model these are the psi particles, which freeze out, and it is equal to (r_g*g_IR)/stat#, where stat# is a statistics number depending on whether this particle is a boson (stat# = 1) or a fermion (stat# = 7/8).
  double N_step;// size of step in terms of extra neutrino families
  double T_pacdr;// [K] PAcDR temperature (today)
  double T_pacdr_wouldbe;// [K] would-be PAcDR temperature (today, had freeze-out not taken place)
  double m_pacdr;// [GeV] mass of massive PAcDR component (psi) that freezes-out
  double a_tr;// transition scale factor, defined to be T_pacdr/m_pacdr
  ...
  //spartacous: new indices
  int index_bg_rho_pacdm;     // PAcDM density
  int index_bg_xa_pacdr;      // x(a) = T_pacdr(a)/m_pacdr
  int index_bg_rho_pacdr;     // PAcDR density
  int index_bg_p_pacdr;       // PAcDR pressure
  int index_bg_nsub_pacdr;    // PAcDR subcomponent number density [eV^3]
  int index_bg_N_pacdr;       // PAcDR extra neutrino families
  int index_bg_w_pacdr;       // PAcDR equation of state w(a)
  int index_bg_cs2_pacdr;     // PAcDR sound speed
  int index_bg_Gamma_pacdr;   // momentum transfer rate from PAcDR to PAcDM
  ...
  //spartacous: booleans for presence of new fluids
  short has_pacdm;     // indicates presence of PAcDM
  short has_pacdr;     // indicates presence of PAcDR
  //spartacous
...
//spartacous
//useful constants
#define _M_Pl_ 2.43532710e18           /**< Reduced Planck Mass in GeV: sqrt((c^5*h/(2*PI))/(8*PI*G)) */
#define _GeV_times_m_ 5.06773121e15   /**< conversion factor: multiply a GeV quantity by this number to get it in units of m^-1 */
#define _kg_over_GeV_ 5.609589118485283e26 /**< conversion factor: multiply a kg quantity to obtain it in units of GeV  */
#define invMpc2_class_density_to_GeV4_conversion (pow(_c_/_Mpc_over_m_,2.)*(3./8./_PI_/_G_)*( _kg_over_GeV_/pow(_GeV_times_m_,3.) )) /* a factor converting the energy densities in CLASS units [Mpc^-2] to natural units [GeV^4] */
//spartacous
