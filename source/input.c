...
int input_read_parameters_species(struct file_content * pfc,
  ...
  class_call(parser_read_string(pfc,"ignore N_UV at BBN",&string1,&flag13,errmsg),
             errmsg,
             errmsg);
  //spartacous_change: log z_tr again
  class_call(parser_read_double(pfc,"z_tr",&param14,&flag14,errmsg),
             errmsg,
             errmsg);
  //spartacous_change
  if (flag13 == _TRUE_){
  ...
  class_test((flag5 == _TRUE_) && (flag11 == _TRUE_) && (flag14 == _TRUE_),// if z_tr was passed AND T_pacdr and m_pacdr were also passed: CONFLICT!
             errmsg,
             "You cannot pass z_tr AND BOTH {T_pacdr, m_pacdm}. The former can be derived from the latter two.");
  class_test((flag12 == _TRUE_) && (flag14 == _TRUE_),// if z_tr was passed AND T_pacdr and m_pacdr were also passed: CONFLICT!
             errmsg,
             "You cannot pass a_tr AND z_tr. The former can be derived from the latter.");
  //spartacous_change:
  ...
  //spartacous_change: set m_psi when z_t is accepted
  else if ((particle_physics == _TRUE_) && (flag14 == _TRUE_))// get m_pacdr from {T_pacdr, a_tr}
    pba->m_pacdr = (pba->T_pacdr*_k_B_/_eV_ * 1.e-9)/pba->a_tr;
  //spartacous_change
